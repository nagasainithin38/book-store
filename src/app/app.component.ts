...
import { Book, cartList } from './books'
...
search(){
  ...
  let newBookList=this.masterBooks.filter(i=>i.title.toLowerCase().includes(this.searchForm?.value?.title.toLowerCase()))
  ...
  addBookTocart(x: Book) {

    let presentIndex = -1
    for (let i = 0; i < this.cartList.length; i++) {
      if (this.cartList[i].book.ISBN == x.ISBN) {
        presentIndex = i
        break
      }
    }
    if (presentIndex == -1) {
      ...
      this.cartList[presentIndex].quantity += 1
    }
